...
```
インストール後、**install-open-notebook.bat**をダブルクリックしてセットアップツールを実行して下さい。  
Docker Desktopがインストールされていない場合は自動でインストールされます。
インストール後、再起動を求められます。  
再起動後、もう一度 **install-open-notebook.bat**を実行すると、インストールの続きが実行されます。  

インストール後はDesktopに **ランチャーが作成される** ので、以降はランチャーからDockerDesktopの起動、OpenNotebookの起動、停止ができます。  

> OpenNotebookの起動後、Modelsから使用するモデルのプロバイダ、APIなどを設定してください。  
>ローカルでAIを実行したい場合はOllamaをインストールする必要があります。[インストール手順はこちら](#ollamaローカル-llmを使う場合)
...
| PCスペック | VRAM (GPU) | RAM (メモリ) | おすすめモデル (Gemma) | おすすめモデル (Qwen) | 備考 |
|:---:|:---:|:---:|---|---|---|
| 💻 低スペック | なし / ~4GB | 8GB | `gemma3:1b` | `qwen3:1.7b` | CPU実行・最低限の会話が可能 |
| 🖥 中スペック | 4〜8GB | 16GB | `gemma3:4b` / `gemma3` | `qwen3:4b` | バランス型・日常使いに十分 |
| 🖥 高スペック | 8〜12GB | 16GB〜 | `gemma3:12b` | `qwen3:8b` | 高品質な回答が可能 |
| 🏢 ハイエンド | 16GB〜 | 32GB〜 | `gemma3:27b` | `qwen3:14b` / `qwen3:32b` | 最高品質・重い処理もOK |

> **VRAMの確認方法**: タスクマネージャー →「パフォーマンス」→「GPU」→「専用GPUメモリ」
> **RAMの確認方法**: タスクマネージャー →「パフォーマンス」→「メモリ」
> ⚠️ VRAM が不足する場合は CPU で実行されますが、速度が大幅に低下します。
> ⚠️ 上の表は目安です。実際の動作はPCの環境によって異なります。
